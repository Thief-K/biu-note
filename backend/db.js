...
export const initDb = async () => {
  try {
    const { stmts } = getDb();
    const countRow = stmts.countMetadata.get();
...
export const saveMetadata = async (filepath, hash, tags, updated_at) => {
  const { stmts } = getDb();
...
export const getMetadata = async (filepath) => {
  const { stmts } = getDb();
  const row = stmts.getMetadata.get(filepath);
...
export const allMetadata = async () => {
  const { stmts } = getDb();
  const rows = stmts.allMetadata.all();
  return rows.map(formatMetaRow);
};

// Delete metadata for a note file
export const deleteMetadata = async (filepath) => {
  const { stmts } = getDb();
