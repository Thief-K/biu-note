...
const getVectorsPath = () => path.join(getBiunoteDir(), 'vectors.json');
...
export const loadVectors = () => {
  ...
  try {
    ...
    console.error('Failed to load vector cache, starting empty:', err.message);
    vectorDb = {};
  }
};
...
export const saveVectors = () => {
  try {
    ...
    console.error('Failed to write vector cache to disk:', err.message);
  }
};
...
export const getEmbedding = async (text, config) => {
...
export const updateVector = async (filepath, content, hash, config) => {
  try {
    ...
    console.error(`Failed to generate embedding for ${filepath}:`, err.message);
  }
};

// Remove vector for deleted file
export const removeVector = (filepath) => {
