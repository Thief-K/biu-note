...
export default function ChatPage() {
  ...
  const handleRemoveNote = (filepath) => {
            ...
            <div className="flex flex-col w-full bg-zinc-900/90 border border-zinc-800 focus-within:border-purple-500/50 rounded-2xl px-3.5 py-2 transition-all">
              ...
              <div className="flex items-end gap-2">
                ...
                <IconButton
                  ...
                  size="sm"
                  shape="rounded-xl"
                  variant={(input.trim() || selectedNotes.length > 0) && !loading && aiConfigured ? 'primary-purple' : 'default'}
                  className={`w-8 h-8 shrink-0 ${!(input.trim() || selectedNotes.length > 0) || loading || !aiConfigured ? 'opacity-40 cursor-not-allowed' : ''}`}
                />
              </div>
            </div>
          </form>
