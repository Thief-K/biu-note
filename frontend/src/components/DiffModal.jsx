...
export default function DiffModal({ data, onCancel, onConfirm }) {
  const { t } = useI18n();
  ...
  const handleRefine = async (e) => {
    ...
    try {
      ...
      setEditedContent(resData.diff_content);
  ...
  const handleSave = () => {
    ...
    const tagsList = editedTags.split(',')
      .map(t => t.trim())
      .filter(Boolean);
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-50 flex items-center justify-center p-4 select-none">
      <div className="w-full max-w-6xl h-[90vh] bg-zinc-900 border border-zinc-700/80 rounded-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        {/* Modal Header Banner */}
        <div className={`p-4 flex items-center justify-between shrink-0 select-none ${
          ...
          <div className="flex items-center gap-2.5">
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
              currentData.action === 'merge' ? 'bg-blue-500/20 text-blue-400' : 'bg-emerald-500/20 text-emerald-400'
            }`}>
        ...
        <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-zinc-800 bg-zinc-950/40 grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 shrink-0">
          <div className="space-y-1">
            <label className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider block">{t('notes.title')}</label>
            <input
              type="text"
              value={editedTitle}
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider block">{t('common.tag')}</label>
            <input
              type="text"
              value={editedTags}
        ...
        <div className="p-4 border-t border-zinc-800 bg-zinc-950 shrink-0 flex flex-col gap-3">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            {/* Refinement input */}
            <form onSubmit={handleRefine} className="flex-1 flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
