...
const extractTitleFromContent = (text) => {
  const heading = (text || '').match(/^\s*#{1,6}\s+(.+)$/m)?.[1] || (text || '').split('\n').find(l => l.trim())?.slice(0, 30);
  return heading ? heading.replace(/[\\/:*?"<>|]/g, '').trim() : '';
};

export default function LiveMarkdownEditor({ filepath, initialNote, onSaved }) {
  ...
  const [content, setContent] = useState(() => (isNew ? '' : (initialNote?.content || '')));
  const [initialContent, setInitialContent] = useState(() => (isNew ? '' : (initialNote?.content || '')));
  const [tags, setTags] = useState(() => (isNew ? [] : (Array.isArray(initialNote?.tags) ? initialNote.tags : [])));
  const [initialTags, setInitialTags] = useState(() => (isNew ? [] : (Array.isArray(initialNote?.tags) ? initialNote.tags : [])));
  const [newTagInput, setNewTagInput] = useState('');
  const [isAddingTag, setIsAddingTag] = useState(false);
  const [isPreview, setIsPreview] = useState(() => !isNew);
  ...
  const scrollToHeading = (h) => {
    ...
    if (isPreview) {
      ...
      if (container && textarea && content) {
        const targetSearch = (typeof h === 'object' && h.raw) ? h.raw.trim() : (typeof h === 'object' ? h.text : '');
        let charIndex = targetSearch ? content.indexOf(targetSearch) : -1;
        if (charIndex === -1 && typeof h === 'object' && h.text) {
